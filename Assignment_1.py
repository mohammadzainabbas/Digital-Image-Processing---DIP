...
def Thresholding(img):
    mean_value = int(np.mean(img))
    print('Mean pixel value is ' + str(mean_value))
    r,c = np.shape(img)
...
def SortSublists(my_list):          #Passing my list to be sort
    ...
    for i in range(my_list.__len__()): #For looping till the length of list
...
img = np.array(cv.imread('gray1.jpg',0))

new_img = Thresholding(img)
img2 = np.array(cv.imread('wb.png',0))
x,l1=ConnectedComponentAnalysis(img2, Connectivity=8)
cv.imshow('Image',x)
cv.waitKey(10000)
cv.destroyAllWindows()
